  ...
  return (
    ...
    >
      <div className="flex flex-col gap-3">
        ...
        {/* Vitals Grid — only Glucose Trend is from the ML model.
            All physical sensors (HR, SpO₂, Temp, Stress, Activity) are
            NOT connected in the current prototype. */}
        <div className="mt-2.5 grid grid-cols-2 gap-2 border-t border-border/20 pt-2.5 sm:grid-cols-3">
          <VitalItem icon={Heart}        label="Heart Rate"    value="N/A" sub="Sensor Offline" status="offline" />
          <VitalItem icon={Wind}         label="SpO₂"         value="N/A" sub="Sensor Offline" status="offline" />
          <VitalItem icon={Thermometer}  label="Skin Temp"    value="N/A" sub="Sensor Offline" status="offline" />
          <VitalItem icon={Brain}        label="Stress Level" value="N/A" sub="Sensor Offline" status="offline" />
          <VitalItem icon={ActivityIcon} label="Activity"     value="N/A" sub="Sensor Offline" status="offline" />
          {/* Only this comes from the actual ML model */}
          <VitalItem
            icon={Sparkles}
            label="Glucose Trend"
            value={trend}
...
interface VitalItemProps {
  ...
  value: string;
  sub?: string;
  status: "normal" | "info" | "warning" | "critical" | "offline";
}

function VitalItem({ icon: Icon, label, value, sub, status }: VitalItemProps) {
  const statusColors = {
  ...
  const subColors: Record<string, string> = {
    "Sensor Offline": "text-destructive/70",
    "Simulated":      "text-amber-500/80",
    "ML Model":       "text-primary/80",
  };

  return (
    <div className={`flex flex-col gap-0.5 rounded-xl border bg-muted/10 p-2 ${status === "offline" ? "border-destructive/20 bg-destructive/5" : "border-border/20"}`}>
      <div className="flex items-center gap-1 text-[9px] text-muted-foreground leading-none">
        <Icon className={`h-3 w-3 shrink-0 ${status === "offline" ? "text-destructive/50" : "text-muted-foreground/80"}`} />
      ...
      {sub && (
        <div className={`text-[9px] leading-none mt-0.5 font-medium ${subColors[sub] || "text-muted-foreground/60"}`}>
          {sub}
        </div>
      )}
    </div>
